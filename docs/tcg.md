...
Emblems are used IN PLACE of Bot Cards. The Emblems represent Bots that appear on the field as a result of a card being activated. These Emblems are not included in a Deck.

When put into play, Emblems are placed on the Bot Zone of the Game Mat. When destroyed, Emblems are returned to a player's hand by card effects, they are also removed from the field. Emblems count toward the 5-card Bot Card Zone limit.


**BUZZ CARDS**

Buzz Cards can only be activated during Main Phases. Buzz Cards are identified by the YELLOW light effects on the edges and borders of the card. To activate a Buzz Card you have to pay Mana except if a Buzz Card has a Mana Cost of 0. The Mana Cost for each Buzz Card is declared by the number in the green colored gem in the top right corner. Once the effect of a Buzz Card is resolved you must put it face-up on the Junk Zone.
...
The Mana Costs are declared by the number in the green colored gem in the top right corner. For every card you want to summon or activate you have to pay the according Mana Costs.
